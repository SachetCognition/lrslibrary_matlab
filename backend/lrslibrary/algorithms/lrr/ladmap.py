"""LADMAP: Linearized ADM with Adaptive Penalty for LRR.

Ports algorithms/lrr/LADMAP/run_alg.m.
...
@register("LRR", "LADMAP", "LADMAP (Lin et al. 2011)", speed_class=2)
class LADMAP(Decomposer):
    """Linearized ADM with Adaptive Penalty for LRR."""
    ...
    def decompose(
        ...
        for _k in range(max_iter):
            ...
            thresh = 1.0 / (mu * eta + 1e-10)
            svp = int(np.sum(s > thresh))
            if svp > 0:
                Z = U[:, :svp] @ np.diag(s[:svp] - thresh) @ Vt[:svp, :]
            else:
                Z = np.zeros((n, n))

"""HoRPCA-S: Higher-order RPCA (Sum of Nuclear Norms).

Ports algorithms/td/HoRPCA-S/run_alg.m.
...
@register("TD", "HoRPCA-S", "HoRPCA-S (Goldfarb and Qin, 2014)", speed_class=3, is_tensor=True)
class HoRPCAS(Decomposer):
    """Higher-order RPCA with Sum of Nuclear Norms."""
    ...
    def decompose(
        ...
        Y = Y / max(norm_two, np.linalg.norm(Y.ravel(), np.inf) / lam)

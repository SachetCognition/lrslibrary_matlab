"""MAMR: Moving Average Matrix Recovery.

Ports algorithms/ttd/MAMR/run_alg.m.
...
@register("TTD", "MAMR", "MAMR (Sobral et al. 2016)", speed_class=2)
class MAMR(Decomposer):
    """Moving Average Matrix Recovery for TTD."""
    ...
    def decompose(
        ...
        # Moving average as initial background estimate
        window = min(10, n)
        bg = np.zeros_like(M)
        for j in range(n):
            start = max(0, j - window // 2)
            end = min(n, j + window // 2 + 1)
            bg[:, j] = np.mean(M[:, start:end], axis=1)
